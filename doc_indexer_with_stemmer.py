"""
Wire Processing Business Capability Indexer with Query Expansion + STEMMING
ENHANCED VERSION with Porter Stemmer for handling word variations
Handles: payment/payments, process/processing, validate/validation, etc.
"""
...
class TextStemmer:
    ...
    def get_word_variations(self, word: str, include_original: bool = True) -> List[str]:
        """
        Get common variations of a word for matching
        E.g., "payment" → ["payment", "payments", "paying"]
        """
        variations = []
        ...
        # Add common suffixes
        base = word.lower().rstrip('s')  # Remove plural
        
        suffixes = ['', 's', 'ing', 'ed', 'ion', 'tion', 'ment', 'er', 'or']
        
        for suffix in suffixes:
            variant = base + suffix
            if variant != word.lower() and len(variant) > 3:
                variations.append(variant)
        
        # Stem all variations to same form
        if self.use_stemming:
            stem = self.stem(word)
            variations = [stem]  # All map to same stem
...
class QueryExpander:
    ...
    def __init__(
        ...
        Args:
            ...
            stemmer: Text stemmer for word normalization
    ...
    def expand_query(
        ...
        Now includes stemmed variations
        """
        result = {
        ...
        # Add stemmed version if available
        if self.stemmer and self.stemmer.use_stemming:
            stemmed_query = self.stemmer.stem_text(query)
            if stemmed_query != query.lower():
                result["expanded_queries"].append(stemmed_query)
                result["expansion_metadata"]["methods_used"].append("stemming")
    ...
    def _basic_expansion(self, query: str) -> List[str]:
        """Level 1: Basic rule-based expansion using synonyms + stemming"""
        expansions = []
...
class FastKeywordExtractor:
    ...
    def extract(self, text: str, max_keywords: int = 20) -> List[Tuple[str, float]]:
        ...
        for term_stem in self.important_terms_stemmed:
            ...
            if count > 0:
                # Store stemmed version for consistency
        ...
        for phrase in capitalized_phrases:
            if len(phrase) > 5:
                if self.stemmer and self.stemmer.use_stemming:
                    phrase_stem = self.stemmer.stem_text(phrase.lower())
                else:
                    phrase_stem = phrase.lower()
                keyword_scores[phrase_stem] += 2.0
        ...
        for kw in capability_keywords:
            if self.stemmer and self.stemmer.use_stemming:
                ...
                # Check if stemmed version is in stemmed text
        ...
        # 6. Extract individual words from multi-word capability keywords
        for kw in capability_keywords:
            words_in_kw = kw.split()
            if len(words_in_kw) > 1:  # Multi-word phrase
                for word in words_in_kw:
                    if len(word) > 3 and word not in self.stopwords:
                        if self.stemmer and self.stemmer.use_stemming:
                            word_stem = self.stemmer.stem(word)
                            count = stemmed_words.count(word_stem)
                        else:
                            word_stem = word
                            count = text_lower.count(word)
                        
                        if count > 0:
                            keyword_scores[word_stem] += 1.5 * math.log1p(count)
        ...
        for term in hyphenated:
            if len(term) > 5:
                if self.stemmer and self.stemmer.use_stemming:
                    term_stem = self.stemmer.stem_text(term)
                else:
                    term_stem = term
                keyword_scores[term_stem] += 1.5
...
class CapabilityMapper:
    ...
    def __init__(self, stemmer: Optional[TextStemmer] = None):
        ...
        for capability, keywords in self.taxonomy.CAPABILITIES.items():
            for kw in keywords:
                # Stem keyword if stemmer available
                if self.stemmer and self.stemmer.use_stemming:
                    kw_stem = self.stemmer.stem_text(kw)
                else:
                    kw_stem = kw
    ...
    def map_to_capabilities(
        ...
        for keyword, kw_score in keywords:
            ...
            for exp_kw in expanded:
                if self.stemmer and self.stemmer.use_stemming:
                    exp_kw_stem = self.stemmer.stem_text(exp_kw)
                else:
                    exp_kw_stem = exp_kw
                
                if exp_kw_stem in self.capability_keywords:
                    for capability in self.capability_keywords[exp_kw_stem]:
...
class WireProcessingIndexer:
    """Fast PDF indexer for wire processing documents - WITH STEMMING"""
    ...
    def __init__(
        ...
        use_stemming: bool = True  # NEW: Enable stemming by default
    ):
        ...
        # Initialize stemmer
        print("Initializing text stemmer...")
        self.stemmer = TextStemmer(use_stemming=use_stemming)
        
        print("Initializing keyword extractor (with stemming)...")
        self.keyword_extractor = FastKeywordExtractor(stemmer=self.stemmer)
        
        print("Initializing capability mapper (with stemming)...")
        ...
        self.metadata_store = []
        self.document_store = []
        self.document_store_stemmed = []  # NEW: Stemmed version for BM25
    ...
    def index_pdfs(self, batch_size: int = 32):
        """Main indexing pipeline - NOW WITH STEMMING"""
        ...
        print("\n=== Phase 1: Extracting Content (WITH STEMMING) ===")
        for idx, pdf_path in enumerate(pdf_files, 1):
            ...
            print(f"  Extracting keywords (stemmed)...")
        ...
        if self.use_embeddings:
            ...
            for i in range(0, len(all_chunks), batch_size):
                ...
                for chunk in batch:
                    ...
                    # Store stemmed version for BM25
                    if self.use_stemming:
            ...
            print("\n=== Phase 3: Storing Metadata (WITH STEMMING) ===")
            for chunk in all_chunks:
                top_capabilities = chunk["capabilities"][:3]
                metadata = {
                    "source_file": chunk["source_file"],
            ...
            print(f"✓ Stored {len(all_chunks)} chunks (keyword search with stemming)")
        ...
        documents_stemmed_path = self.index_path / "documents_stemmed.pkl"
        ...
        if self.use_stemming:
            with open(documents_stemmed_path, 'wb') as f:
                pickle.dump(self.document_store_stemmed, f)
            print(f"✓ Saved stemmed documents to: {documents_stemmed_path}")
...
class WireProcessingSearcher:
    """Fast search using BM25 with enhanced query processing + STEMMING"""
    ...
    def __init__(
        ...
        if self.use_stemming:
            documents_stemmed_path = self.index_path / "documents_stemmed.pkl"
            if documents_stemmed_path.exists():
                with open(documents_stemmed_path, 'rb') as f:
                    self.document_store_stemmed = pickle.load(f)
                print("✓ Loaded stemmed documents for BM25")
            else:
                # Fallback: stem on the fly
                print("⚠ Stemming documents on-the-fly for BM25...")
        ...
        if self.use_embeddings and EMBEDDINGS_AVAILABLE and FAISS_AVAILABLE:
            ...
            print("⚡ Using keyword/BM25 search with stemming")
            self.faiss_index = None
            self.embedder = None
        
        # Build BM25 index with stemmed documents
        ...
        if self.enable_query_preprocessing:
            print(f"✓ Enhanced query preprocessing enabled (with stemming)")
    ...
    def search(
        ...
        """
        Enhanced search with query preprocessing, expansion, and STEMMING
        NOW: "payments" matches "payment", "processing" matches "process"
        """
        if verbose:
            print(f"\n{'='*80}")
            print(f"Query: {query}")
        ...
        # Step 1: Preprocess query (remove stopwords, extract core terms, STEM)
        original_query = query
        search_query = query
        core_terms = []
        phrases = []
        ...
        if self.enable_query_preprocessing:
            ...
            if verbose:
                ...
                print(f"  Cleaned: {query_analysis['cleaned_query']}")
                if self.use_stemming:
                    print(f"  Stemmed: {query_analysis['stemmed_query']}")
        ...
        if use_query_expansion and self.query_expander:
            ...
            expanded_info = self.query_expander.expand_query(
                search_query,  # Use cleaned/stemmed query
                query_capabilities
            )
    ...
    def _enhanced_bm25_search(
        ...
        """Enhanced BM25 search with better scoring + STEMMING"""
        
        # Collect all query variants (already stemmed)
        ...
        for variant_idx, variant in enumerate(query_variants):
            # Tokenize variant (already stemmed if stemming enabled)
        ...
        # Add phrase matching bonus (on original text)
        if phrases:
...
def main():
    """Example usage - WITH STEMMING SUPPORT"""
    import argparse
    
    parser = argparse.ArgumentParser(
        description="Wire Processing Indexer - WITH STEMMING (payment→payments, process→processing)"
    )
    ...
    print("=" * 70)
    print("Wire Processing Indexer - WITH STEMMING")
    print("Handles: payment/payments, process/processing, validate/validation")
    print("=" * 70)
    print()
...
print("\n" + "=" * 70)
print("✓ Wire Processing Indexer - ENHANCED WITH STEMMING")
print("  • Porter Stemmer enabled (payment→payments, process→processing)")
print("  • Consistent matching across word variations")
print("  • Enhanced query preprocessing with stemming")
print("  • BM25 search on stemmed documents")
print("  • Query expansion with stemming support")
print("=" * 70)
