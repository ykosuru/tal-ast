...
class DataProcessor:
    """Process training data with enhanced ACE learning and directory traversal"""
    ...
    def load_transactions_from_path(self, path: str) -> Dict:
        """Load transactions from a file or directory"""
        all_transactions = {}
    ...
    def _load_file(self, filepath: str) -> Dict:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                logger.warning(f"Unexpected data format in {filepath}: {type(data)}")
    ...
    def _load_directory(self, dirpath: str) -> Dict:
        ...
        logger.info(f"Found {len(json_files)} JSON files in directory tree")
        ...
        for filepath in sorted(json_files):
            ...
            duplicates = set(all_transactions.keys()) & set(file_transactions.keys())
            if duplicates:
                logger.warning(f"Found {len(duplicates)} duplicate transaction IDs from {filepath}")
                # Add file prefix to avoid collisions
                file_prefix = os.path.splitext(os.path.basename(filepath))[0]
                for txn_id in duplicates:
                    new_id = f"{file_prefix}_{txn_id}"
                    file_transactions[new_id] = file_transactions.pop(txn_id)
                    logger.debug(f"  Renamed duplicate {txn_id} to {new_id}")
        ...
        logger.info(f"Loaded total of {len(all_transactions)} transactions from {file_count} files")
        return all_transactions
    
    def load_and_process(self, path: str, max_transactions: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray, List[Dict]]:
        """Load and process training data from file or directory"""
        ...
        # Optionally limit number of transactions for testing
        if max_transactions and len(all_transactions) > max_transactions:
            logger.info(f"Limiting to {max_transactions} transactions for processing")
        ...
        for txn_id, txn_data in all_transactions.items():
            ...
            if 'source' in txn_data:
                file_sources[txn_data['source']] += 1
            ...
            if not txn_data['ace'] or len(txn_data['ace']) == 0:
                empty_ace_count += 1
                continue
...
# ============================================================================
# CLI (Enhanced with Directory Support)
# ============================================================================
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog='''
Examples:
  Train a model from a single file:
    python ace_repair_predictor.py train --input training_data.json
  
  Train a model from a directory of files:
    python ace_repair_predictor.py train --input ./training_data/
  
  Train with limited transactions for testing:
    python ace_repair_predictor.py train --input ./training_data/ --max_txns 1000
  
  Predict repairs for a payment:
    python ace_repair_predictor.py predict --input payment.json
  
  Predict repairs for all payments in a directory:
    python ace_repair_predictor.py predict --input ./payments/ --output ./results/
  
  Analyze a directory without training:
    python ace_repair_predictor.py analyze --input ./training_data/
        '''
    )
    ...
    if args.command == 'train':
        ...
        # Handle max transactions
        if args.max_txns:
            logger.info(f"Training with maximum {args.max_txns} transactions")
            # Modify the trainer's processor to handle max_txns
        ...
        if os.path.isfile(args.input):
            ...
            if args.output:
                save_prediction_results(results, args.output, args.input)
            ...
            for filepath in json_files:
                try:
                    ...
                    if args.output:
                        output_path = os.path.join(args.output, f"predictions_{filename}")
                        save_prediction_results(results, output_path, filepath)
                    
                    if not args.batch:
                        logger.info(f"Processed {filename}: {len(results['predicted_repairs'])} repairs predicted")
        ...
        # Analyze data without training
        analyzer = DataProcessor()
        
        logger.info(f"Analyzing data from {args.input}")
        
        # Load transactions
        all_transactions = analyzer.load_transactions_from_path(args.input)
        
        # Limit transactions if specified
        if args.max_txns and len(all_transactions) > args.max_txns:
            all_transactions = dict(list(all_transactions.items())[:args.max_txns])
            logger.info(f"Limited to {args.max_txns} transactions for analysis")
        
        # Perform analysis
        repair_frequency = Counter()
        repair_combinations = Counter()
        entity_repair_map = defaultdict(Counter)
        repair_codes = Counter()
        
        for txn_id, txn_data in all_transactions.items():
            if not isinstance(txn_data, dict) or 'ace' not in txn_data:
                continue
        ...
        logger.info(f"Unique repairs found: {len(repair_frequency)}")
        
        logger.info(f"\nTop 20 Most Frequent Repairs:")
        for repair_id, count in repair_frequency.most_common(20):
            percentage = (count / len(all_transactions)) * 100
            logger.info(f"  {repair_id}: {count} ({percentage:.1f}%)")
        
        logger.info(f"\nRepair Code Distribution:")
        for code, count in repair_codes.most_common():
            logger.info(f"  Code {code}: {count}")
        
        logger.info(f"\nTop 10 Repair Combinations:")
        for combo, count in repair_combinations.most_common(10):
            logger.info(f"  {' + '.join(combo)}: {count} times")
        
        # Save analysis if output specified
        if args.output:
            analysis_report = {
                'total_transactions': len(all_transactions),
                'unique_repairs': len(repair_frequency),
                'repair_frequency': dict(repair_frequency),
                'repair_codes': dict(repair_codes),
                'repair_combinations': {
                    ' + '.join(combo): count 
                    for combo, count in repair_combinations.most_common(20)
                },
                'repair_taxonomy': analyzer.ace_learner.repair_taxonomy,
                'lookup_tables_stats': {
                    'bic_to_name': len(analyzer.ace_learner.lookup_tables['bic_to_name']),
                    'bic_to_address': len(analyzer.ace_learner.lookup_tables['bic_to_address']),
                    'clearing_to_bic': len(analyzer.ace_learner.lookup_tables['clearing_to_bic']),
                    'country_codes': len(analyzer.ace_learner.lookup_tables['country_extractions'])
                }
            }
...
def save_prediction_results(results: Dict, output_path: str, input_path: str):
    ...
    results['metadata'] = {
        ...
        'total_repairs_predicted': len(results['predicted_repairs'])
    }
