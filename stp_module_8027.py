#!/usr/bin/env python3
"""
Test rules for 8027 against actual IFMLs.
8027 = IBAN/Account Validation

Description: IBAN or account number validation failed
...
Usage:
    python test_8027_rules_v2.py --data-dir /path/to/ifml/data --limit 10000
    python test_8027_rules_v2.py --show-docs   # Feature documentation
    python test_8027_rules_v2.py --show-party  # Party reference guide
"""
...
FEATURE_DOCS = """
================================================================================
FEATURE DOCUMENTATION FOR 8027 (IBAN/Account Validation)
================================================================================

8027 fires when: IBAN or account number validation failed

KEY FEATURES:
┌────────────────────────────────┬────────────────────────────────────────────┐
│ Feature                        │ Meaning                                    │
├────────────────────────────────┼────────────────────────────────────────────┤
│ iban_checksum_valid_count      │ Count of parties with valid IBAN checksums │
│ iban_format_valid_count        │ Count of parties with valid IBAN format    │
│ bnf_nch_sources                │ Number of NCH (National Clearing House) ... │
│ bnf_has_iban                   │ Beneficiary account has IBAN               │
│ bnf_account_length             │ Length of beneficiary account number       │
│ dbt_is_bic_derivable           │ Debtor bank BIC can be derived from acco... │
└────────────────────────────────┴────────────────────────────────────────────┘
...
def check_rules(features: Dict) -> Tuple[bool, List[str]]:
    """
    Check if 8027 should fire based on extracted rules.
    ...
    # Rule 1: Valid IBAN checksums found (validation was performed)
    if (get('iban_checksum_valid_count', 0) or 0) > 0:
        reasons.append(f"iban_checksum_valid_count={get('iban_checksum_valid_count')}")
    
    # Rule 2: Any party has IBAN in their account
    for prefix in ['bnf_', 'cdt_', 'dbt_', 'orig_']:
        if get(f'{prefix}has_iban', False):
            party = prefix.upper().rstrip('_')
            reasons.append(f"{party}: account has IBAN")
    
    # Rule 3: Beneficiary has NCH codes (domestic routing)
    if (get('bnf_nch_sources', 0) or 0) > 0:
        reasons.append(f"BNF: has NCH sources={get('bnf_nch_sources')}")
    
    # Rule 4: Beneficiary account missing (no IBAN and no account number)
    if not get('bnf_has_iban', False) and (get('bnf_account_length', 0) or 0) == 0:
        reasons.append("BNF: no IBAN and account_length=0")
    
    # Rule 5: Beneficiary present but missing identifiers
    if get('bnf_present', False) and not get('bnf_has_iban', False) and not get('bnf_has_bic', False):
        reasons.append("BNF: present but missing account IBAN and bank BIC")
...
def main():
    ...
    # Report
    print("\n" + "="*70)
    print(f"TEST RESULTS: {TARGET_CODE} - IBAN/Account Validation")
    print("="*70)
